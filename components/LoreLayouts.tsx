...
export const DossierStyle = ({ entries }: { entries: any[] }) => (
  <div className="bg-black text-green-400 p-8 font-mono">
    ...
    <div className="space-y-6">
      {entries.map((entry) => (
        <div key={entry.slug} className="border border-green-600 p-4 rounded hover:bg-green-900/10 transition">
          <h2 className="text-xl font-bold uppercase tracking-wide">{entry.title}</h2>
          <p className="text-xs opacity-70 mb-1">Classification: <span className="font-semibold">{entry.classification}</span></p>
          <p className="text-xs opacity-70 mb-2">Date: {entry.date}</p>
          <p className="text-sm text-green-300 line-clamp-3">{entry.excerpt}</p>
          <a href={`/lore/${entry.slug}`} className="inline-block mt-2 text-green-400 underline">Read Dossier</a>
        </div>
      ))}
    </div>
  </div>
      ...
      <tbody>
        {entries.map((entry) => (
          <tr key={entry.slug} className="border-b border-green-800 hover:bg-green-900/10">
            ...
            <td className="py-2">
              <a href={`/lore/${entry.slug}`} className="text-green-300 underline">Open File</a>
            </td>
          </tr>
        ))}
      </tbody>
