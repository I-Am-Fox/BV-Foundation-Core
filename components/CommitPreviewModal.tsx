...
export default function CommitPreviewModal({ filename, onConfirm, onCancel }: Props) {
  const [meta, setMeta] = useState<{ title: string; classification: string; asset: string } | null>(
    null
  );
      ...
      <div className="bg-black border border-green-500 p-6 rounded-md text-green-300 font-mono w-full max-w-lg">
        ...
        {meta ? (
          <div className="text-sm space-y-1 mb-4">
            ...
            <p>
              <strong>Title:</strong> {meta.title || '[Untitled]'}
            </p>
            <p>
              <strong>Classification:</strong> {meta.classification || '[Unclassified]'}
            </p>
            <p>
              <strong>Asset:</strong> {meta.asset || '[Unknown Asset]'}
            </p>
          </div>
        ) : (
        ...
        <div className="flex justify-end gap-2 text-xs">
          <button onClick={onCancel} className="px-3 py-1 bg-red-700 hover:bg-red-600">
            Cancel
          </button>
          <button onClick={onConfirm} className="px-3 py-1 bg-green-700 hover:bg-green-600">
            Confirm Commit
          </button>
        </div>
      </div>
