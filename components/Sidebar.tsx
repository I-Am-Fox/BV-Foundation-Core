    ...
    <div className="flex">
      <aside className="w-72 bg-zinc-900 text-white p-4 border-r border-green-700 min-h-screen overflow-y-auto">
        ...
        <nav className="space-y-3 text-sm">
          ...
          <a href="/lore" className="block hover:text-green-400">
            Lore
          </a>
          <a href="/characters" className="block hover:text-green-400">
            Characters
          </a>
          <a href="/timeline" className="block hover:text-green-400">
            Timeline
          </a>
        </nav>
        ...
        <nav className="space-y-3 text-sm">
          {Object.entries(structure).map(([classification, assets]) => {
            const color =
              COLORS[classification.toUpperCase() as keyof typeof COLORS] ?? 'text-white';
            return (
                  ...
                  {openGroups[classification] && (
                    ...
                    >
                      {Object.entries(assets).map(
                        ([asset, entries]: [string, { slug: string; title: string }[]]) => (
                          <div key={asset}>
