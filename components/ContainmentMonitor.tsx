...
export default function ContainmentMonitor({ entries }: { entries: { slug: string; frontMatter: { asset: string } }[] }) {
    const [selection, setSelection] = useState<{ title: string; status: string } | null>(null);

    useEffect(() => {
        if (entries.length > 0) {
            const randomEntry = entries[Math.floor(Math.random() * entries.length)];
            const randomStatus = STATUS_OPTIONS[Math.floor(Math.random() * STATUS_OPTIONS.length)];
            setSelection({ title: randomEntry.frontMatter.asset, status: randomStatus });
        }
    }, [entries]);
